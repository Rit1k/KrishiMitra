...
def detect_disease():
    ...
    try:
        ...
        logger.info("Prediction successful: %s (confidence: %f)", disease, confidence)
        ...
        logger.error("Error in disease detection: %s", str(e))
...
def chat():
    try:
        ...
        if not (user and user.get('subscribed', False)):
            message_count = db.messages.count_documents({'user_id': user_id})
            if message_count >= 5:
                return jsonify({
                    'error': 'Message limit reached',
                    'response': "You've reached the limit of free messages. Please subscribe to continue chatting."
                }), 403
        
        # Prepare agricultural context for Gemini AI
